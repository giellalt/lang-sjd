...
All Kildin, Ter, Pite and Ume texts %<----Micha, please check that that's correct, irgendwie weiß ich nicht was dahin gehört
will be available and searchable using the Giellatekno search interface (similar to the North Saami interactive text corpus available from \url{http://giellatekno.uit.no/text.en.html}). These corpora will be completely parallel (with the translations that we have) and will additionally be tagged for the time alignment included in the respective ELAN files in order to permit linking to DOBES/ELAR media files.
