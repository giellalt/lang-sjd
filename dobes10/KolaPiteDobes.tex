...
\section{Abstract} %JW: 2,5 Seiten
...
\label{SaamiLgs}
%\includegraphics[width=0.9\textwidth]{SaamiLgs.jpeg} %JW: das Bild bringt TexShop jedes mal zum Stoppen, das nervt, musste ich für mich rausnehmen.
\end{center}
\end{figure}
...
Together with the Kola Saami Youth organization \textit{\textbf{Sām' nūraš}} (one of our cooperation partners) we will attempt to start up a Kildin Saami Wikipedia, as members of \textit{\textbf{Sām' nūraš}} have expressed interest in this. In doing so will use T.\,Trosterud's experiences working with the Nynorsk and North Saami Wikipedias to our advantage. %Sollen wir hier auch schreiben, dass Trond aktiver Wikipedianer für die bedrohte Sprache Nynorsk ist?
% nja, vielleicht das Apertium gut für nb > nn funktioniert
% aber wie früher gesagt:
% nordsaamisch wp ist echt schlecht, und eigentlich nur ein nfrastruktur für nächste generation saamen
%aber vielleicht müssen wir das hier im antrag nicht so genau schreiben, die gutachter werden das nicht wissen
% nein, natürlich, ich will nicht spoil a nice text, nur aber sagen, es kann dort probleme sein
% wp auf Komi geht doch ganz gut
...
% Wir können die einsprachige multimedia Datenbank aus dem LEXUS-Projekt ins Wikipdiaformat exportieren und haben damit schon einen guten Start für die Wikipedia. Die muss danach natürlich von Sprechern weiter gepflegt werden. Aber wir können außerdem versuchen mit der machine-translation Seiten aus der nordsaamischen und vielleicht sogar russischen Wikipedia ins Kildin zu übersetzen%Das habt ihr doch für Nynorsk auch mal so gemacht oder?
%Damit bietet die neue Kildin-Wikipedia auch einen hervorragenden testing ground für die machine-translation
...
i) start programming correction tools for Pite (J.\,Wilbur, C.\,Gerstenberger, in cooperation with the Norwegian Saami parliament workgroup \url{http://divvun.no})\\ %JW: geht das?, %TT: der speller wird ebensogut als der transducer, kein problem
%i) Complete lexical coverage of the analyzers, working on corpus interface (all team members).\\
